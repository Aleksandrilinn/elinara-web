...
export default function VCPage() {
  ...
  // Inputs (Base Case Initial Values)
  ...
  // Scenario Logic (Presets)
  const scenarios: any = {
    base: { label: "Base Case (Neutral)", tam: 100000000000, quota: 10.0, margem: 20.0, multiplo: 15.0, desconto: 30.0, diluicao: 7.0 },
    bear: { label: "Bear Case (Pessimistic)", tam: 50000000000, quota: 5.0, margem: 10.0, multiplo: 10.0, desconto: 40.0, diluicao: 10.0 },
    bull: { label: "Bull Case (Optimistic)", tam: 250000000000, quota: 25.0, margem: 30.0, multiplo: 25.0, desconto: 25.0, diluicao: 3.0 }
  };
  ...
  const applyScenario = (key: string) => {
      ...
      // Auto-calculate on scenario change
      ...
      <div className="relative z-10 pt-28 px-4 max-w-[1400px] mx-auto flex-grow w-full">
        ...
        <div className="text-center mb-10 w-full">
            ...
            <div className="space-y-4 text-gray-400 text-sm md:text-base font-light leading-relaxed max-w-3xl mx-auto">
                <p>
                    The Venture Capital method is the answer to financial theory&apos;s extreme uncertainty. 
                    Unlike traditional models requiring history, this &quot;Top-Down&quot; approach starts in the future (TAM) and discounts execution risk and dilution to determine a startup&apos;s rational value today.
                </p>
            </div>
            ...
            <div className="w-fit mx-auto relative rounded-2xl bg-[#080808] border border-white/10 overflow-hidden hidden md:block shadow-2xl">
                ...
                <div className="px-6 py-3 border-b border-white/5 flex items-center justify-between relative z-10 bg-black/20 backdrop-blur-sm gap-12">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono uppercase tracking-wider text-gray-300">VC Method Logic</span>
                    </div>
                ...
                <div className="p-8 overflow-x-auto scrollbar-hide relative z-10">
                    <div className="flex items-center justify-center min-w-max gap-2">
                        <GlowBlock label="TAM" sub="Total Market" color="white" />
                        ...
                        <GlowBlock label="Revenue" sub="Future Sales" color="blue" />
                        <ColoredOperator icon={<X size={12}/>} color="text-gray-600" />
                        <GlowBlock label="Margin" sub="Profitability" color="gray" />
                        <ColoredOperator icon={<Equal size={12}/>} color="text-gray-600" />
                        <GlowBlock label="Exit Val" sub="Exit Value" color="green" />
                        <ColoredOperator icon={<Divide size={12}/>} color="text-red-600" />
                        <GlowBlock label="Risk" sub="VC Discount" color="red" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto pb-20">
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-[#0F0F0F] border border-white/10 p-6 rounded-2xl shadow-xl relative z-30">
                    <label className="text-xs font-mono text-gray-500 uppercase tracking-widest mb-4 block">1. Base Scenario</label>
                    <div className="grid grid-cols-3 gap-2 mb-6">
                        {Object.keys(scenarios).map(key => (
                            ...
                            >
                                {scenarios[key].label.split(' ')[0]} {/* Ex: Base, Bear, Bull */}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="space-y-4">
                        <FancyInput label="TAM (Total Market $)" value={inputs.tam} onChange={(v:any) => setInputs({...inputs, tam: v})} step={1000000000} />
                        <ControlSlider label="Market Share (%)" value={inputs.quota} setValue={(v:any) => setInputs({...inputs, quota: v})} min={0} max={100} step={0.5} />
                        <ControlSlider label="Operating Margin (%)" value={inputs.margem} setValue={(v:any) => setInputs({...inputs, margem: v})} min={0} max={80} step={1} />
                        <div className="grid grid-cols-2 gap-4">
                             <FancyInput label="Exit Multiple (x)" value={inputs.multiplo} onChange={(v:any) => setInputs({...inputs, multiplo: v})} step={1} />
                             <FancyInput label="Target Year (Exit)" value={inputs.target_year} onChange={(v:any) => setInputs({...inputs, target_year: v})} step={1} />
                        </div>
                    </div>
                    ...
                    <button onClick={() => handleCalculate()} disabled={loading} className="w-full mt-6 bg-white text-black hover:bg-gray-200 font-bold py-4 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                        {loading ? <span className="animate-pulse">Calculating...</span> : <>Calculate Value <Rocket size={18}/></>}
                    </button>
                </div>

                {/* 2. RISK & DILUTION */}
                <div className="bg-[#0F0F0F] border border-white/10 p-5 rounded-2xl shadow-xl relative z-20">
                    <h3 className="text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-4 border-b border-white/5 pb-2">2. Risk & Dilution</h3>
                    <div className="space-y-5">
                        <ControlSlider label="VC Discount (Risk %)" value={inputs.desconto} setValue={(v:any) => setInputs({...inputs, desconto: v})} min={10} max={60} step={1} />
                        <ControlSlider label="Annual Dilution (%)" value={inputs.diluicao} setValue={(v:any) => setInputs({...inputs, diluicao: v})} min={0} max={30} step={0.5} />
                        <div className="grid grid-cols-1 gap-4 pt-2">
                             <FancyInput label="Current Shares (Qty)" value={inputs.acoes_atuais} onChange={(v:any) => setInputs({...inputs, acoes_atuais: v})} />
                        </div>
                    </div>
                </div>
                ...
                {/* 3. SURVIVAL */}
                <div className="bg-[#0F0F0F] border border-white/10 p-5 rounded-2xl shadow-xl relative z-20">
                    <h3 className="text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-4 border-b border-white/5 pb-2">3. Survival (Runway)</h3>
                    <div className="grid grid-cols-2 gap-4">
                        <FancyInput label="Current Cash ($)" value={inputs.caixa_atual} onChange={(v:any) => setInputs({...inputs, caixa_atual: v})} />
                        <FancyInput label="Annual Burn ($)" value={inputs.burn_anual} onChange={(v:any) => setInputs({...inputs, burn_anual: v})} />
                    </div>
                </div>
            </div>
                ...
                {data && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                        ...
                        <div className={`p-4 rounded-xl border flex items-center gap-4 ${data.survival.status === 'danger' ? 'bg-red-900/10 border-red-500/30 text-red-400' : data.survival.status === 'warning' ? 'bg-yellow-900/10 border-yellow-500/30 text-yellow-400' : 'bg-green-900/10 border-green-500/30 text-green-400'}`}>
                            ...
                            <div>
                                <p className="font-bold text-sm">Estimated Runway: {data.survival.runway.toFixed(1)} Years</p>
                                <p className="text-xs opacity-80">
                                    {data.survival.status === 'danger' ? "CRITICAL: Imminent capital need." : data.survival.status === 'warning' ? "WARNING: Fundraising focus needed soon." : "HEALTHY: Time to execute."}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-[#0F0F0F] border border-white/10 p-8 rounded-2xl relative overflow-hidden group">
                                ...
                                <p className="text-xs font-mono text-purple-400 uppercase mb-2">Target Price (Diluted)</p>
                                <p className="text-6xl font-bold text-white tracking-tight">${data.metrics.target_price.toFixed(2)}</p>
                                <p className="text-[10px] text-gray-500 mt-2 uppercase">Value per share today (risk adjusted)</p>
                            </div>

                            <div className="bg-[#0F0F0F] border border-white/10 p-8 rounded-2xl flex flex-col justify-center relative overflow-hidden">
                                <p className="text-xs font-mono text-gray-500 uppercase mb-4">Future Metrics ({inputs.target_year})</p>
                                <div className="space-y-3">
                                    <div className="flex justify-between border-b border-white/5 pb-2">
                                        <span className="text-sm text-gray-400">Target Revenue</span>
                                        <span className="font-mono text-white">{formatMoney(data.metrics.receita_alvo)}</span>
                                    </div>
                                    <div className="flex justify-between border-b border-white/5 pb-2">
                                        <span className="text-sm text-gray-400">NOPAT (Profit)</span>
                        ...
                        <div className="bg-[#0F0F0F] border border-white/10 rounded-2xl p-6">
                            <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2 mb-6"><Layers size={14}/> Value Decomposition</h3>
                            
                            <div className="space-y-2">
                                <WaterfallStep step="1" label="Future Value (Exit)" sub={`In ${inputs.target_year} with {inputs.multiplo}x multiple`} value={data.metrics.exit_value} icon={<Target size={16}/>} />
                                <div className="flex justify-center py-2"><ArrowRight className="rotate-90 text-gray-700" size={16}/></div>
                                
                                <WaterfallStep step="2" label="Present Value (PV)" sub={`Discounted at ${inputs.desconto}% (VC Risk)`} value={data.metrics.pv_equity} color="text-blue-400" bg="bg-blue-900/10" border="border-blue-500/20" icon={<Clock size={16}/>} />
                                <div className="flex justify-center py-2"><ArrowRight className="rotate-90 text-gray-700" size={16}/></div>
                                ...
                                <div className="w-full md:w-2/3 mx-auto p-4 rounded-xl border border-red-500/20 bg-red-900/10 flex justify-between items-center relative">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-mono text-red-400 uppercase border border-red-400/20 px-1.5 rounded">Step 3</span><PieChart size={16} className="text-red-400"/></div>
                                        <p className="font-bold text-white text-lg">Dilution Effect</p>
                                        <p className="text-xs text-gray-500 mt-0.5">New shares issued to finance cash burn</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-gray-500 strike-through line-through opacity-50">No Dilution: ${data.metrics.price_no_dilution.toFixed(2)}</p>
...
// --- VISUAL COMPONENTS (Design Matched) ---
